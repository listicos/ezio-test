...
typedef NS_ENUM(NSInteger, EMUserActionState) {
    ...
    /**
     * User select to get OTP with BioFp
     */
    EMUserActionStateGetOtpWithBioFp
};
...
- (void)updateUIControlsAfterProvisioning
{
    dispatch_async(dispatch_get_main_queue(), ^{
        self.btnUpgradeToMultiAuthMode.enabled = YES;
        self.btnGetOTPUsingPIN.enabled = YES;
        self.btnGetOTPUsingBioFP.enabled = NO;
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        self.btnUpgradeToMultiAuthMode.enabled = NO;
        self.btnGetOTPUsingPIN.enabled = YES;
        if ([self.example isBioFingerprintSupportedAndConfigured]) {
            self.btnGetOTPUsingBioFP.enabled=YES;
        }
    });
}
...
- (void)showPinpad
{
    ...
    @catch (EMException *exception) {
        if ([EMActivationException isEqualToString:[exception name]]) {
            UIAlertView *pinInput = [[UIAlertView alloc] initWithTitle:@"Oath Example" message:@"Please enter PIN:" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Ok", nil];
...
- (IBAction)btnGetOtpUsingPINTouchedUpInside:(id)sender
{
...
- (IBAction)btnGetOtpUsingBioFPTouchedUpInside:(id)sender
{
    // Make sure the token is already migrated to multi auth mode
    if (![self.example isMultiAuthModeEnabled]) {
        // Invalid state, token has to be upgraded to multi auth mode before we could verify with BioFp mode
        return;
    }
    
    id<EMBioFingerprintAuthInput> bioFpAuthInput = [self.example authenticateUserWithMessage:@"Verify Fingerprint"];
    if (!bioFpAuthInput) {
        // e.g. User cancel, etc.
        return;
    }
    ...
    // Set user action state
    _userState = EMUserActionStateGetOtpWithBioFp;
    
    // Generate OTP
    [self.example generateOtpWithAuthInput:bioFpAuthInput];
    
    // Security Guideline: GEN09. Sensitive data leaks
    // The AuthInput must be wiped ASAP.
    [bioFpAuthInput wipe];
}
